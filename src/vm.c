...
// Initialise VM state.
int bee_init(bee_word_t *pc, bee_uword_t ssize, bee_uword_t dsize)
{
    bee_R.pc = pc;
    bee_R.dp = 0;
...
int bee_init_defaults(bee_word_t *pc)
{
...
bee_word_t bee_run(void)
{
    ...
    for (;;) {
#ifdef HAVE_MIJIT
        mijit_bee_run(bee_jit, (mijit_bee_registers *)(void *)&bee_R);
#endif
